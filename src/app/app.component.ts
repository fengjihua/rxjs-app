...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // 煮蛋器
    const eggMachine = Observable.create((xiaoMing) => {
    ...
    // 面包机
    const toastMachine = Observable.create((xiaoMing) => {
    ...
    // 咖啡机
    const cafeMachine = Observable.create((observer) => {
      console.log('8点' + this.minute + '分 煮咖啡');
      this.minute += 2;
      observer.next('8点' + this.minute + '分 [咖啡完成]');
    });

    // 同步
    console.log('[5分钟GET H5] - 同步的第一天：');
    this.minute = 0;
    eggMachine.subscribe((message) => {
      console.log(message);

      cafeMachine.subscribe((message) => {
        console.log(message);

        toastMachine.subscribe((message) => {
          console.log(message);
        });
      });
    });
